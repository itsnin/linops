// dev_toolchain rendering
// shows which toolchains will be installed
// rust shows a note that it uses rustup not apt
pub fn render(
    ...
    let warning = crate::core::color::Color::Warning;

    let names = super::state::State::names();
    grid.write_str(area.x, area.y, "Development Toolchain", accent, bg, true);
    grid.write_str(
        area.x,
        area.y + 2,
        "install programming language toolchains and dev tools",
        muted,
        bg,
        false,
    );

    for (i, (name, desc, method)) in names.iter().enumerate() {
        ...
        // show install method note for rustup
        if *method == "rustup" {
            grid.write_str(
                area.x + 2,
                row + 1,
                &format!("{} (via rustup not apt)", desc),
                warning,
                bg,
                false,
            );
