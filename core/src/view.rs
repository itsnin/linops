...
pub fn view(
    ...
    let mut grid = crate::grid::Grid::new(state.width, state.height);
    ...
    // fill background
    grid.fill(
        crate::rect::Rect::new(0, 0, state.width, state.height),
        theme.bg,
    );

    // layout: topbar 3 rows then content then statusbar 1 row
    let (topbar, rest) = crate::rect::Rect::new(0, 0, state.width, state.height).split_v(3);
...
fn render_topbar(
    ...
    let mode_str = match state.mode {
        ...
        crate::mode::Mode::Search => " SEARCH ",
        crate::mode::Mode::Confirm => " CONFIRM ",
        crate::mode::Mode::Help => " HELP ",
        crate::mode::Mode::Task => " RUNNING ",
    };
    grid.write_str(
...
fn render_sidebar(
    ...
    for (row_offset, (i, cat)) in (0..).zip(crate::state::ALL_CATEGORIES.iter().enumerate()) {
...
fn render_help(
    ...
    let lines = [
        ...
        "left right switch task in category",
        "up down   navigate items in task",
        "enter     activate item",
        "esc       go back",
...
fn render_statusbar(
    ...
    let hint = match state.mode {
        crate::mode::Mode::Normal => {
            "tab:category  left right:task  up down:item  enter:activate  /:search  ?:help  ctrl c:quit"
        }
        crate::mode::Mode::Search => {
            "type to search  up down:navigate  enter:select  esc:cancel"
        }
        crate::mode::Mode::Confirm => "y:confirm  n:decline  esc:cancel",
        crate::mode::Mode::Help => "esc or ?:close",
        crate::mode::Mode::Task => "esc or ctrl c:cancel",
    };
